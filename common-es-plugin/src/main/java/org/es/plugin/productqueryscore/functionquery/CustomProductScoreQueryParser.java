...
public class CustomProductScoreQueryParser implements QueryParser{
private static Logger logger=Logger.getLogger(CustomProductScoreQueryParser.class);
...
//                   e.printStackTrace();
        ...
        if (filterFunctions.size() == 1 && filterFunctions.get(0).filter == null) {
            CustomProductScoreQuery theQuery = new CustomProductScoreQuery(query, filterFunctions.get(0).function);
